...
function getAllDirectors(movies) {
  let retorno=[];
  let ab = movies.map(function (element) {
...
function howManyMovies(movies) {
  let ab = movies.filter(function (element) {
...
function scoresAverage(movies) {
  let retorno=0;
   retorno = movies.reduce(function (a, b){
    //console.log(b.score); 
    return a+b.score;
  },0);
  return (retorno/movies.length).toFixed(2);
}
...
function dramaMoviesScore(movies) {
  ...
  let retorno = ab.reduce(function (a, b){
    return a+b.score;
  },0);
  return (retorno/ab.length).toFixed(2);
}
...
function orderByYear(movies) {
  retorno = movies;
  return retorno.sort((a,b) => a.year-b.year);
...
function orderAlphabetically(movies) {
  let aux=movies;
  aux.sort((a,b) => a.title.localeCompare(b.title))
  let retorno = aux.map(function (element) {
    //console.log(element.director);  
    return element.title;
    });
  if(retorno.length>=20){
    return retorno.slice(0,20);
  }
  return retorno;
}
